...
<html >
   <head>
		...
		body{ background:url(<?php echo e(url('/images/bg.jpg')); ?>) center center no-repeat #333; background-attachment:fixed; background-size:cover; height:100%}
    ...
    <!--<body style="background-image: linear-gradient(to top, rgba(0, 0, 0, 0.7), rgba(0, 0, 0, 0.2)), url('../images/bg2.jpg'); background-size:cover; background-repeat:no-repeat; background-attachment:fixed; background-position:center center">-->
    <body style="">

		<div class="container">
        <form id="entry" method="POST" action="<?php echo e(route('resetpassword', app()->getLocale())); ?>" autocomplete="off">
		<?php echo csrf_field(); ?>
            <a href="<?PHP echo url("/") ?>"><img src="<?php echo e(url('/images/logo-transparent.png')); ?>" class="logo"/></a>
            ...
            <div class="top" style="margin-bottom:0px; padding-bottom:8px; border-radius:4px">
                <input id="email" type="email" name="email" class="field required" autocomplete="off" required value="" placeholder="Email" style=" margin-bottom:0px; border-radius:4px; border-bottom: 0px none"/>
                <span class="user material-icons">mail_outline</span>
            </div>
            
            <div class="submit">
                <button type="submit"><?php echo e(__('btnSendMail')); ?></button>
            </div>
            <a href="<?PHP echo url(app()->getLocale()."/login") ?>" class="register"><?php echo e(__('btnHaveAccount')); ?></a>
        </form>
        </div>
		</div>
