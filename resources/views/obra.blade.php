...
@section('content')
    ...
    <div class="wrappercontainer">
        ...
        <div class="container">
            ...
            <form class="mt-45" id="obra-form" method="post" accept-charset="utf-8" enctype="multipart/form-data">
                @csrf
                <div class="main-primary col-sm-8 col-md-8 col-xs-12 br-5 p15 mb-30 col-sm-offset-2">
                    <h3 class="header3">Bienvenido a la @if($tipo=="show") edición @else captura @endif de obra @if($tipo=="show") {{$obra->id}} @endif</h3>                    
                    
                    <p class="descriptionsection">Por favor llene todos los campos marcados <mark></mark> para poder brindar un mejor servicio.</p>
                     
                    @if($tipo=="show")
                    
                    <a href="{{route('listadoobras')}}" class="btn btn-primary btn-block" style="float: left; width: 50%; margin-top: 0px;margin-right: 10px">← Listado de Obras</a>
                    <a href="{{route('crearobra')}}" class="btn btn-warning btn-block" style="float: left; width: calc(50% - 10px); margin-top: 0px">+ Crear obra</a>
                    @else
                    <a href="{{route('listadoobras')}}" class="btn btn-primary btn-block" style="float: left; width: 50%">← Listado de Obras</a>
                    @endif
                    ...
                    <div class="inputfield">                        
                        ...
                        <label>Nombre de la obra <mark></mark></label>
                    </div> 
                    <div class="inputfield">                        
                        <input type="text" class="texto"  minlength="2"  name="descripcion" required />
                        <label>Descripción de la obra <mark></mark></label>
                    </div> 
                    <div class="inputfield">                        
                    ...
                    <div class="col-sm-7 col-xs-12 pl0"> 
                        <label>Tipo de obra <mark></mark></label>
                        <select class="form-control mb-30 select2-single" required name="tipo">
                            ...
                            <option value="Hidráulica e Infraestructura">Hidráulica e Infraestructura</option>
                            <option value="Vivienda">Vivienda</option>         
                        </select>
                    </div> 
                    <div class="col-sm-5 col-xs-12 pr0"> 
                        <label>Etapa de la obra<mark></mark></label>
                    ...
                    <div class="col-sm-8 col-xs-12 pr0">
                        ...
                        <select class="form-control mb-30 select2-single" required name="area_responsable">
                            <option value="">Seleccione un valor</option>
                            <option value="Dirección de Desarrollo Social">Dirección de Desarrollo Social</option>
                            <option value="Dirección de Modernización, Innovación y Gobierno Abierto">Dirección de Modernización, Innovación y Gobierno Abierto</option>
                    ...
                    <div class="col-sm-12 col-xs-12 pl0" style="margin-bottom: 10px">
                        ...
                        <div class="inputfield actived">     
                            <input type="text" class="texto" name="lat_lng" placeholder="Sin ubicación seleccionada, arrastra el pin dentro del mapa indicando la ubicación de la obra..."  required minlength="2" style="pointer-events: none;">
                            <label>Latitud y longitud de la obra <mark></mark></label>
                        </div>
                    </div>  



                    <label class="titlesection"><b>4</b><span>Fotos de obra</span></label>  

                    @if($tipo=="show") 
                        <div class="col-xs-12  pl0">
                            <div class="col-xs-2 pl0" style="margin-bottom: 20px">
                                @if($obra->imagen_1!="")<img src="{{$obra->imagen_1}}" style="width: 100%; border-radius:4px">@endif
                            </div>
                        ...
                        <div class="col-xs-12  pl0">
                            <div class="col-xs-2 pl0" style="margin-bottom: 20px">
                                @if($obra->imagen_2!="")<img src="{{$obra->imagen_2}}" style="width: 100%; border-radius:4px">@endif
                            </div>
                            <div class="col-xs-10 pr0" style="margin-bottom: 20px">    
                        ...
                        <div class="col-xs-12  pl0">
                            <div class="col-xs-2 pl0" style="margin-bottom: 20px">
                                @if($obra->imagen_3!="")<img src="{{$obra->imagen_3}}" style="width: 100%; border-radius:4px">@endif
                            </div>
                            <div class="col-xs-10 pr0" style="margin-bottom: 20px">  
                        ...
                        <div class="col-xs-12  pl0">
                            <div class="col-xs-2 pl0" style="margin-bottom: 20px">
                                @if($obra->imagen_4!="")<img src="{{$obra->imagen_4}}" style="width: 100%; border-radius:4px">@endif
                            </div>
                            <div class="col-xs-10 pr0" style="margin-bottom: 20px">
                    ...
                    @else
                        <div class="col-sm-12 col-xs-12 pl0" style="margin-bottom: 20px">  
                            <label>Foto principal (1) (JPG,JPEG,PNG,GIF - Máx: 10MB) <mark></mark></label>                    
                            <input type="file" name="imagen_1" required/>                        
                        </div> 
                        <div class="col-sm-12 col-xs-12 pl0" style="margin-bottom: 20px">  
                            <label>Foto secundaria (2) (JPG,JPEG,PNG,GIF - Máx: 10MB)</label>                    
                            <input type="file" name="imagen_2" />                        
...
@section('page-js-script')
  ...
  <script>
    ...
    $(document).ready(function(){
      ...
      @if($tipo=="show")
        $("[name='nombre']").val('{!!$obra->nombre!!}');
        $("[name='descripcion']").val('{!!$obra->descripcion!!}');
        $("[name='compromiso']").val('{!!$obra->compromiso!!}');
        $("[name='tipo']").val('{!!$obra->tipo!!}');
        $("[name='etapa']").val('{!!$obra->etapa!!}');
        $("[name='comuna']").val('{!!$obra->comuna!!}');
        $("[name='area_responsable']").val('{!!$obra->area_responsable!!}');        
        $("[name='porcentaje_avance']").val('{!!$obra->porcentaje_avance!!}');
        $("[name='mano_obra']").val('{!!$obra->mano_obra!!}');
        $("[name='plazo_meses']").val('{!!$obra->plazo_meses!!}');
        $("[name='benficiarios']").val('{!!$obra->benficiarios!!}');
        $("[name='fecha_inicio']").val('{!!$obra->fecha_inicio!!}');
        $("[name='fecha_fin_inicial']").val('{!!$obra->fecha_fin_inicial!!}');
        $("[name='monto_contrato']").val('{!!$obra->monto_contrato!!}');
        $("[name='nro_contratacion']").val('{!!$obra->nro_contratacion!!}');
        $("[name='contratacion_tipo']").val('{!!$obra->contratacion_tipo!!}');
        $("[name='licitacion_anio']").val('{!!$obra->licitacion_anio!!}');
        $("[name='financiamiento']").val('{!!$obra->financiamiento!!}');
        $("[name='calle_1']").val('{!!$obra->calle_1!!}');
        $("[name='direccion']").val('{!!$obra->direccion!!}');
        $("[name='barrio']").val('{!!$obra->barrio!!}');
        $("[name='lat_lng']").val('{!!$obra->lat!!},{!!$obra->lng!!}'); //setear en mapa tambien
        //foto1
        //foto2
        //foto3
        //foto4
      ...
      $("#obra-form").on('submit', function(){
        var form = $('#obra-form')[0];
        ...
        $.ajax({
            ...
            error: function(xhr, resp, text) {
                ...
                $(".loading-main").fadeOut();
                $(".responsemessage").addClass("errorresponse");
                $(".responsemessage").addClass("showed").html("Ocurrió un error guardando la obra: "+error+". Línea: "+linea).slideDown();
            }
        });
        return false;
      });
    ...
    function initMap() {    
        var map = new google.maps.Map(document.getElementById('map_canvas'), {
            zoom: 18,
            center: new google.maps.LatLng(@if($tipo=="show") {{$obra->lat}},{{$obra->lng}} @else 19.1952535, -96.1478962 @endif),
            mapTypeId: google.maps.MapTypeId.ROADMAP
        });
        var myMarker = new google.maps.Marker({
            position: new google.maps.LatLng(@if($tipo=="show") {{$obra->lat}},{{$obra->lng}} @else 19.1952535, -96.1478962 @endif),
            draggable: true
        });
